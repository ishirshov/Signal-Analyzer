...
/*!
 * \brief Функция выполняет умножение двух векторов X Y и
 ...
 */
inline void ipp_mul(const Ipp8u *src1, const Ipp8u *src2, Ipp8u *dst, Ipp32u size)
{
    IppStatus status = ippsMul_8u_Sfs(src1, src2, dst, size, 0);
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp16u *src1, const Ipp16u *src2, Ipp16u *dst, Ipp32u size)
{
    IppStatus status = ippsMul_16u_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp16s *src1, const Ipp16s *src2, Ipp16s *dst, Ipp32u size)
{
    IppStatus status = ippsMul_16s_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp32s *src1, const Ipp32s *src2, Ipp32s *dst, Ipp32u size)
{
    IppStatus status = ippsMul_32s_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp16sc *src1, const Ipp16sc *src2, Ipp16sc *dst, Ipp32u size)
{
    IppStatus status = ippsMul_16sc_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp32sc *src1, const Ipp32sc *src2, Ipp32sc *dst, Ipp32u size)
{
    IppStatus status = ippsMul_32sc_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp32f *src1, const Ipp32f *src2, Ipp32f *dst, Ipp32u size)
{
    IppStatus status = ippsMul_32f(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp64f *src1, const Ipp64f *src2, Ipp64f *dst, Ipp32u size)
{
    IppStatus status = ippsMul_64f(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp32fc *src1, const Ipp32fc *src2, Ipp32fc *dst, Ipp32u size)
{
    IppStatus status = ippsMul_32fc(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp64fc *src1, const Ipp64fc *src2, Ipp64fc *dst, Ipp32u size)
{
    IppStatus status = ippsMul_64fc(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp8u *src1, const Ipp8u *src2, Ipp16u *dst, Ipp32u size)
{
    IppStatus status = ippsMul_8u16u(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_mul(const Ipp32f *src1, const Ipp32fc *src2, Ipp32fc *dst, Ipp32u size)
{
    IppStatus status = ippsMul_32f32fc(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
