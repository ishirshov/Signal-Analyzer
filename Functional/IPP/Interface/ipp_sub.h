...
/*!
 * \brief Функция выполняет операцию вычитания двух векторов X Y и
 ...
 */
inline void ipp_sub(const Ipp8u *src1, const Ipp8u *src2, Ipp8u *dst, Ipp32u size)
{
    IppStatus status = ippsSub_8u_Sfs(src1, src2, dst, size, 0);
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp16u *src1, const Ipp16u *src2, Ipp16u *dst, Ipp32u size)
{
    IppStatus status = ippsSub_16u_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp16s *src1, const Ipp16s *src2, Ipp16s *dst, Ipp32u size)
{
    IppStatus status = ippsSub_16s(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp32s *src1, const Ipp32s *src2, Ipp32s *dst, Ipp32u size)
{
    IppStatus status = ippsSub_32s_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp16sc *src1, const Ipp16sc *src2, Ipp16sc *dst, Ipp32u size)
{
    IppStatus status = ippsSub_16sc_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp32sc *src1, const Ipp32sc *src2, Ipp32sc *dst, Ipp32u size)
{
    IppStatus status = ippsSub_32sc_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp32f *src1, const Ipp32f *src2, Ipp32f *dst, Ipp32u size)
{
    IppStatus status = ippsSub_32f(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp64f *src1, const Ipp64f *src2, Ipp64f *dst, Ipp32u size)
{
    IppStatus status = ippsSub_64f(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp32fc *src1, const Ipp32fc *src2, Ipp32fc *dst, Ipp32u size)
{
    IppStatus status = ippsSub_32fc(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sub(const Ipp64fc *src1, const Ipp64fc *src2, Ipp64fc *dst, Ipp32u size)
{
    IppStatus status = ippsSub_64fc(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
