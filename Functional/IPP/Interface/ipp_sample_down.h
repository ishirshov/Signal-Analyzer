...
#ifndef IPP_SAMPLE_DOWN_H
#define IPP_SAMPLE_DOWN_H
...
/*!
 * \brief Функция позволяет понизить частоту дискретизации сигнала X
 ...
 * \param[in] factor    Значение фактора понижения частоты дискретизации.
 ...
 */
inline void ipp_sample_down(const Ipp16s *src, Ipp32u src_sz,
                            Ipp16s *dst, Ipp32s *dst_sz,
                            Ipp32s factor, Ipp32s *phase)
{
    IppStatus status = ippsSampleDown_16s(src, src_sz, dst, dst_sz, factor, phase);
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sample_down(const Ipp32f *src, Ipp32u src_sz,
                            Ipp32f *dst, Ipp32s *dst_sz,
                            Ipp32s factor, Ipp32s *phase)
{
    IppStatus status = ippsSampleDown_32f(src, src_sz, dst, dst_sz, factor, phase);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sample_down(const Ipp64f *src, Ipp32u src_sz,
                            Ipp64f *dst, Ipp32s *dst_sz,
                            Ipp32s factor, Ipp32s *phase)
{
    IppStatus status = ippsSampleDown_64f(src, src_sz, dst, dst_sz, factor, phase);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sample_down(const Ipp16sc *src, Ipp32u src_sz,
                            Ipp16sc *dst, Ipp32s *dst_sz,
                            Ipp32s factor, Ipp32s *phase)
{
    IppStatus status = ippsSampleDown_16sc(src, src_sz, dst, dst_sz, factor, phase);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sample_down(const Ipp32fc *src, Ipp32u src_sz,
                            Ipp32fc *dst, Ipp32s *dst_sz,
                            Ipp32s factor, Ipp32s *phase)
{
    IppStatus status = ippsSampleDown_32fc(src, src_sz, dst, dst_sz, factor, phase);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_sample_down(const Ipp64fc *src, Ipp32u src_sz,
                            Ipp64fc *dst, Ipp32s *dst_sz,
                            Ipp32s factor, Ipp32s *phase)
{
    IppStatus status = ippsSampleDown_64fc(src, src_sz, dst, dst_sz, factor, phase);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}

#endif // IPP_SAMPLE_DOWN_H
