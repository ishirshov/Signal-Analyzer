...
/*!
 * \brief Функция выполняет операцию сложения двух векторов X Y и
 *        результат сохраняет в вектор Z.
 * \param[in] src1 Исходный вектор с 8 разрядными целыми числами X.
 * \param[in] src2 Исходный вектор с 8 разрядными целыми числами Y.
 * \param[out] dst  Конечный вектор с 8 разрядными целыми числами Z.
 * \param[in] size Размер src1, src2 и dst вектора.
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp8u *src1, const Ipp8u *src2, Ipp8u *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_8u_Sfs(src1, src2, dst, size, 0);
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp16u *src1, const Ipp16u *src2, Ipp16u *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_16u_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp16s *src1, const Ipp16s *src2, Ipp16s *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_16s(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp32s *src1, const Ipp32s *src2, Ipp32s *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_32s_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp16sc *src1, const Ipp16sc *src2, Ipp16sc *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_16sc_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \brief Функция выполняет операцию сложения двух векторов X Y и
 *        результат сохраняет в вектор Z.
 * \param[in] src1 Исходный вектор с 32 разрядными целыми комплексными числами X.
 * \param[in] src2 Исходный вектор с 32 разрядными целыми комплексными числами Y.
 * \param[out] dst  Конечный вектор с 32 разрядными целыми комплексными числами Z.
 * \param[in] size Размер src1, src2 и dst вектора.
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp32sc *src1, const Ipp32sc *src2, Ipp32sc *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_32sc_Sfs(src1, src2, dst, size, 0);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp32f *src1, const Ipp32f *src2, Ipp32f *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_32f(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp64f *src1, const Ipp64f *src2, Ipp64f *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_64f(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp32fc *src1, const Ipp32fc *src2, Ipp32fc *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_32fc(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp64fc *src1, const Ipp64fc *src2, Ipp64fc *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_64fc(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
 ...
 * \exception std::runtime_error Ошибка в ходе выполнения функции.
 */
inline void ipp_add(const Ipp8u *src1, const Ipp8u *src2, Ipp16u *dst, Ipp32u size)
{
    IppStatus status = ippsAdd_8u16u(src1, src2, dst, size);
    if(status != ippStsNoErr) throw std::runtime_error(ippGetStatusString(status));
}
