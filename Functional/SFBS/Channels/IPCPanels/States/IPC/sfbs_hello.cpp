...
#include "sfbs_hello.h"

#include "Functional/SFBS/Channels/IPCPanels/isomorph_panel.h"

#include <stdexcept>

sfbsHello::sfbsHello(IsomorphPanel *panel)
    :_panel(panel)
    ,_is_exec(false)
{
    if(!_panel)
...
void sfbsHello::sendMessage()
{
    if(!_is_exec) {
        ipc_sfbs_send_msg(_panel->local_channel_id, IPC_CMD_SFBS_HELLO, 0, 0);
        _is_exec = true;
    }
}

void sfbsHello::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case IPC_CMD_SFBS_REPLY:
    {
        ...
        if(reply->reply_cmd == IPC_CMD_SFBS_HELLO) {
            if(reply->reply_status == REPLY_STATUS_OK) {
                printDebug(QString("I received initialized 'HELLO' message from channel: %1.").arg(_panel->remote_channel_id));

                _panel->state = _panel->sfbs_get_info;
                _is_exec = false;
            }
            else _panel->state = _panel->sfbs_no_state;
        }
        else {
    ...
    if(libsfbs_ch_buffer_release(_panel->remote_channel_id, msg) != SFBS_SUCCESS)
