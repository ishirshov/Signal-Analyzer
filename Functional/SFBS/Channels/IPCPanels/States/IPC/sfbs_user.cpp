...
sfbsUser::sfbsUser(IsomorphPanel *panel)
    :_panel(panel)
{
    if(!_panel)
...
void sfbsUser::sendMessage()
{
    // Nothing to do.
}

void sfbsUser::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    {
        printDebug(QString("I received IPC reply command from channel: %1").arg(_panel->remote_channel_id));

        sfbs_cmd_reply_msg_data_t *reply = 0;
        reply = reinterpret_cast<sfbs_cmd_reply_msg_data_t*>(ipc_msg->data);

        if(reply->reply_cmd == IPC_CMD_SFBS_USER) {
            printDebug("I received device type info message");

            if(reply->reply_status == REPLY_STATUS_OK) {
                _panel->ipcDataReceive(reply->data);
            }
            else {
