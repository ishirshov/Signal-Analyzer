...
applyConfig::applyConfig(FPGAPanel *_panel)
    :panel(_panel)
{
    if(!_panel)
...
void applyConfig::sendMessage()
{
    ...
    fpga_user_ipc_send(panel->user_channel_id,
                       panel->user_task_id,
                       ADC_CMD_APPLY_CONFIG,
                       &values,
                       sizeof(dac_adc_control_ipc_cmd_value_msg_t));
}

void applyConfig::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_APPLY_CONFIG)
        {
            ...
            case REPLY_STATUS_OK:
            {
                if(reply_data->replyParamOne == panel->devices->currentIndex())
                {
                    ...
                    panel->stream_control->setEnabled(true);
                }
                break;
            }
            case REPLY_STATUS_ERROR:
            {
                QTextStream(&warning_text) << QObject::tr("Can not apply config for: [ Device: ") << reply_data->replyParamOne << " ]";
                break;
            }
