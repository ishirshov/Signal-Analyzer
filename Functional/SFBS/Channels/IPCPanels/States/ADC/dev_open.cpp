...
devOpen::devOpen(FPGAPanel *_panel)
    :panel(_panel)
{
    if(!_panel)
...
void devOpen::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_OPEN_DEV)
        {
            ...
            case REPLY_STATUS_OK:
            {
                if(reply_data->replyParamOne == panel->devices->currentIndex())
                {
                    ...
                    panel->devices->setEnabled(false);
                    panel->tasks_control->setEnabled(false);
                    panel->device_control->setText(QObject::tr("Close"));

                    panel->state = panel->get_dev_freq;
                }
                break;
            }
            case REPLY_STATUS_ERROR:
            {
                QTextStream(&warning_text) << QObject::tr("Can not open device: [ Device: ") << reply_data->replyParamOne << " ]";
