...
#include "get_ch_per_one_device.h"
#include "Functional/SFBS/Channels/IPCPanels/fpga_panel.h"
#include <stdexcept>

getChPerOneDevice::getChPerOneDevice(FPGAPanel *_panel)
    :panel(_panel)
{
    if(!_panel)
...
void getChPerOneDevice::sendMessage()
{
    fpga_user_ipc_send(panel->user_channel_id,
                       panel->user_task_id,
                       ADC_CMD_GET_CH_PER_ONE_DEV, 0, 0);
}

void getChPerOneDevice::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_GET_CH_PER_ONE_DEV)
        {
            switch (fpga_reply->status) {
            case REPLY_STATUS_OK:
            {
                for(int i = 0; i < reply_data->replyParamOne; ++i)
                    panel->channels->addItem(QString("%1").arg(i), i);
                break;
            }
            case REPLY_STATUS_ERROR:
            {
                QTextStream(&warning_text) << QObject::tr("[ Device can not give number of channels per one device. ]");
                break;
            }
