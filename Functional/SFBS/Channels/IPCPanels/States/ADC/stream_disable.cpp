...
#include "stream_disable.h"
#include "Functional/SFBS/Channels/IPCPanels/fpga_panel.h"
#include <stdexcept>

streamDisable::streamDisable(FPGAPanel *_panel)
    :panel(_panel)
{
    if(!_panel)
...
void streamDisable::sendMessage()
{
    ...
    fpga_user_ipc_send(panel->user_channel_id,
                       panel->user_task_id,
                       ADC_CMD_STREAM_DISABLE,
                       &values,
                       sizeof(dac_adc_control_ipc_cmd_value_msg_t));
}

void streamDisable::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_STREAM_DISABLE)
        {
            ...
            case REPLY_STATUS_OK:
            {
                if(reply_data->replyParamOne == panel->devices->currentIndex())
                {
                    if(reply_data->replyParamTwo == panel->channels->currentIndex())
                    {
                        ...
                        panel->stream_control->setText(QObject::tr("Enable"));
                        panel->channel_control->setEnabled(true);
                        panel->apply_control->setEnabled(true);
                        panel->types->setEnabled(true);
                        panel->sampfreq_box->setEnabled(true);                        
                    }
                }
                break;
            }
            ...
            {
                QTextStream(&warning_text) << QObject::tr("Can not disable stream: [ Channel: ") << reply_data->replyParamTwo
