...
void setDevFreq::messageReceive(void *msg)
{
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_SET_DEV_FREQ)
        {
            ...
            case REPLY_STATUS_OK:
            {
                ...
                {
                    QMetaObject::invokeMethod(panel, "samplingFrequencyChanged", Q_ARG(ulong, ulong(reply_data->replyParamTwo)));
                    panel->sampfreq_box->setText(QString("%1").arg(reply_data->replyParamTwo));
                    panel->state = panel->apply_config;
                }
                break;
            }
            case REPLY_STATUS_ERROR:
            {
                QTextStream(&warning_text) << QObject::tr("Can not set device frequency: [ Device: ") << reply_data->replyParamOne
                                           << QObject::tr(", Freq: ") << reply_data->replyParamTwo << " ]";
