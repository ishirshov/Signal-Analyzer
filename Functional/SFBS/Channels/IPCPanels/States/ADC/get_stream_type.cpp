...
#include "get_stream_type.h"
#include "Functional/SFBS/Channels/IPCPanels/fpga_panel.h"
#include <stdexcept>

getStreamType::getStreamType(FPGAPanel *_panel)
    :panel(_panel)
{
    if(!_panel)
...
void getStreamType::sendMessage()
{
    ...
    values.cmdParamThree = 0;

    fpga_user_ipc_send(panel->user_channel_id,
                       panel->user_task_id,
                       ADC_CMD_GET_STREAM_TYPE,
                       &values,
                       sizeof(dac_adc_control_ipc_cmd_value_msg_t));
}

void getStreamType::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_GET_STREAM_TYPE)
        {
            ...
            {
                QTextStream(&warning_text) << QObject::tr("Can not get stream type: [ Channel: ") << reply_data->replyParamTwo
