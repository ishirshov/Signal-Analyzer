...
void getDevFreq::sendMessage()
{
    ...
    fpga_user_ipc_send(panel->user_channel_id,
                       panel->user_task_id,
                       ADC_CMD_GET_DEV_FREQ,
                       &values,
                       sizeof(dac_adc_control_ipc_cmd_value_msg_t));
}

void getDevFreq::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_GET_DEV_FREQ)
        {
            ...
            {
                QTextStream(&warning_text) << QObject::tr("Can not get device frequency: [ Device: ") << reply_data->replyParamOne << " ]";
                break;
            }
