...
chOpen::chOpen(FPGAPanel *_panel)
    :panel(_panel)
{
    if(!_panel)
...
void chOpen::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_OPEN_CH)
        {
            ...
            case REPLY_STATUS_OK:
            {
                if(reply_data->replyParamOne == panel->devices->currentIndex())
                {
                    if(reply_data->replyParamTwo == panel->channels->currentIndex())
                    {
                        ...
                        panel->channel_control->setText(QObject::tr("Close"));
                        panel->device_control->setEnabled(false);
                        ...
                        panel->state = panel->get_stream_t;
                    }
                }
                break;
            }
            ...
            {
                QTextStream(&warning_text) << QObject::tr("Can not open: [ Channel: ") << reply_data->replyParamTwo
