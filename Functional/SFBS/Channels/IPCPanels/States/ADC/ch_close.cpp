...
chClose::chClose(FPGAPanel *_panel)
    :panel(_panel)
{
    if(!_panel)
...
void chClose::messageReceive(void *msg)
{
    if(!msg) return;
    ...
    case DAC_ADC_IPC_CMD_REPLY:
    {
        ...
        if(fpga_reply->for_cmd == ADC_CMD_CLOSE_CH)
        {
            ...
            case REPLY_STATUS_OK:
            {
                if(reply_data->replyParamOne == panel->devices->currentIndex())
                {
                    if(reply_data->replyParamTwo == panel->channels->currentIndex())
                    {
                        ...
                        panel->channel_control->setText(QObject::tr("Open"));
                        panel->device_control->setEnabled(true);
                        panel->channels->setEnabled(true);

                        panel->apply_control->setEnabled(false);
                        panel->types->setEnabled(false);
                        panel->sampfreq_box->setEnabled(false);
                        panel->stream_control->setEnabled(false);
                        panel->format->setText(QObject::tr("Not set"));
                    }
                }
                break;
            }
            ...
            {
                QTextStream(&warning_text) << QObject::tr("Can not close: [ Channel: ") << reply_data->replyParamTwo
