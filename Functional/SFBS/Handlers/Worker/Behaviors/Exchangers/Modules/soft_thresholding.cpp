...
#include "soft_thresholding.h"

SoftThresholding::SoftThresholding()
...
void SoftThresholding::function(Ipp32f *data, int size)
{
...
void SoftThresholding::compute(Ipp32f *src, int size)
{
    // Check size
    if(size > _zeros.size())
    {
    ...
    Ipp32f *abs_p = _tmp2.data();

    ipp_sign(src, sign_p, size);
    ipp_abs(src, abs_p, size);
    ipp_sub_c(abs_p, delta, abs_p, size);
    ipp_amax(abs_p, _zeros.data(), abs_p, size);
    ipp_mul(sign_p, abs_p, src, size);
}
